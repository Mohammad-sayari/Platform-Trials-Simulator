...
# forest plot for multilevel analysis
...
# Define the CRT data simulation functions
crtdata_simulation <- function(nt, n_schools_treated, np, ns, sigma, ICC, sigmaPret, B0, B1, es, seed, attrition_rates) {
  ...
  # Create the base data structure with pupils and schools
  data <- expand.grid(pupils = 1:np, schools = 1:ns)
  data$pupils <- 1:nrow(data)
  
  #  Initialize treatment assignment and assign treatment groups to schools
  ...
  remaining_schools <- setdiff(remaining_schools, control_schools)  
  
  for (i in 1:nt) {
    treated_schools <- sample(remaining_schools, n_schools_treated[i + 1])
    data[[interventions]][data$schools %in% treated_schools] <- i  
    remaining_schools <- setdiff(remaining_schools, treated_schools)  
  }
  
  #  Generate pre-test scores 
  prets <- "pretest"
  ...
  #  Handle attrition for control and treatment groups
  non_attrition_idx <- 1:nrow(data) 
  
  for (i in 0:nt) {
    group_idx <- which(data[[interventions]] == i)
    attrition_rate <- attrition_rates[i + 1]  
    attrition_size <- round(length(group_idx) * attrition_rate)
    
    if (attrition_size > 0) {
      attrition_idx <- sample(group_idx, attrition_size)
      data[attrition_idx, posts] <- NA  
      non_attrition_idx <- setdiff(non_attrition_idx, attrition_idx)  
    }
  }
  
  #  Generate residual errors and cluster-level random effects
  e_ij <- rnorm(length(non_attrition_idx), mean = 0, sd = sigma)  
  sigmab <- sqrt(ICC / (1 - ICC) * sigma^2)  
  b_i_full <- rnorm(ns, mean = 0, sd = sigmab)  
  b_i <- b_i_full[data$schools[non_attrition_idx]]  
  
  # Create treatment effects for each group
  ...
  # Compute post-test scores using the combined formula
...
mstdata_simulation <- function(nt, tpi, np, ns, sigma, sigmab0, sigmab1, sigmaPret, B0, B1, es, seed, attrition_rates) {
  ...
  # Assign control group based on tpi[1] (control percentage)
  ...
  # Assign treatment groups to the remaining pupils
  ...
  # Use model.matrix to create treatment effect 
...
# Define the SRT data simulation functions
srtdata_simulation <- function(nt, tpi, np, sigma, sigmaPret, B0, B1, es, seed, attrition_rates) {
  ...
  # Create the base data structure with participants
  data <- data.frame(ID = 1:np)  
  ...
  # Generate residual errors for non-attrited individuals
  e_ij <- rnorm(length(non_attrition_idx), mean = 0, sd = sigma)
...
# futility function for CRT data
crtfutility <- function(data, post_vars = "post1", intervention_column = "treatments", Random = "schls", Nsim = 2000, 
  ...
  futility_decisions <- numeric(length(interventions))  
...
# futility function for MST data
...
# futility function for SRT data
...
# Superiority function for CRT data
...
# Superiority function for MST data
...
# Superiority function for SRT data
...
add_new_treatmentsrt <- function(existing_data, new_pupils, es, attrition_rate, 
  ...
  new_data <- existing_data[1:new_pupils, ]  
  new_data[[pupils_col]] <- new_pupil_ids  
  new_data[[intervention_col]] <- new_treatment_num  
...
# Function to plot Posterior probabilities for CRT data
...
# Function to plot Posterior probabilities for MST data
...
# Function to plot Posterior probabilities for SRT data
...
#function for multilevel analysis
