...
server <- function(input, output, session) {
    ...
    tryCatch({
      ...
      result <- if (input$futSimType == "CRT") {
        crtfutility(
          ...
          covariates = covariates  # Pass covariates directly
        )
  ...
  observe({
    ...
    updateSelectInput(session, "random_var_sup", choices = names(data))  # Random variable
  ...
  superiorityData <- eventReactive(input$analyzeSuperiority, {
    ...
    tryCatch({
      ...
      covariates <- input$covariates_sup  # Multiple selected covariates
  ...
  analysisResult <- eventReactive(input$runAnalysis, {
    ...
    tryCatch({
      ...
      result <- run_analysis(
        ...
        covariates = covariates  # Pass the selected covariates
      )
  ...
  importedNewTreatmentData <- reactive({
    ...
    # Debugging: Print column names after data is loaded
